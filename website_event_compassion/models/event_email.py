        ...
        for scheduler in self.filtered(lambda s: not s.mail_sent):
            ...
            if missing_registrations:
                mail_registration_ids = [
                    (0, 0, {"registration_id": reg.id}) for reg in missing_registrations
                ]
...
class EventMailRegistration(models.Model):
    ...
    def execute(self):
        ...
        for email in self:
            if email.mail_sent:
                continue
            # if the communication fail by either an uncaught exception or is in the
            # state failure skip this mail and dont mark it as sent
