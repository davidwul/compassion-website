...
function clickHandler() {
  ...
  const page = $("#page").val();
  if (page == "1") {
    const participantId = document.querySelector(
    ...
    if (direct_to_sponsorship != "checked") {
      a_step_02.href = `/project/${a_step_02.getAttribute(
        "project"
      )}/donation?participant=${participantId.value}&page=2`;
      a_step_02.target = "";
    } else {
      a_step_02.href = document.querySelector(
        'input[name="sponsorship_url_' + participantId.value + '"]'
      ).value;
      a_step_02.target = "_blank";
    }
  } else {
    const participantId = $("#participant").val();
    const a_step_01 = document.getElementById("a_step_01");
    a_step_01.href = `/project/${a_step_01.getAttribute(
      "project"
    )}/donation?participant=${participantId}&page=1`;
    a_step_01.target = "";
    // Enable or disable fund according to the choice of the participant
    if (participantId) {
      const products_number = document.querySelector(
        'input[name="product_goal_' + participantId + '"]'
      ).value;
      const sponsorships_number = document.querySelector(
        'input[name="sponsorship_goal_' + participantId + '"]'
      ).value;

      // Used for automatic selection if only one type of support is chosen
      const select_product = products_number && !sponsorships_number;
      const select_sponsorship = sponsorships_number && !products_number;
    ...
    submit.disabled = !(donationType && donationType.value && participantId);

    if (!submit.disabled) {
      if (donationType.value === "product") {
        a.href = `/project/${a.getAttribute(
          "project"
        )}/donation/form/${participantId}?page=3`;
        a.target = "";
      }

      if (donationType.value === "sponsorship") {
        a.href = document.querySelector(
          'input[name="sponsorship_url_' + participantId + '"]'
        ).value;
        a.target = "_blank";
      }
    }
