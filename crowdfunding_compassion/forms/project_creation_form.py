...
class ProjectCreationStep2(models.AbstractModel):
    ...
    _form_fields_hidden = [
        "product_id",
        "participant_product_number_goal",
    ...
    def form_before_create_or_update(self, values, extra_values):
        ...
        if not product_goal and not sponsorship_goal:
            ...
            product_goal
            and int(product_goal) < 0
            or sponsorship_goal
            and int(sponsorship_goal) < 0
        ):
...
class ProjectCreationStep3(models.AbstractModel):
    ...
    partner_image = fields.Binary(
        "Profile picture", help="Upload a profile picture, square 800 x 800px"
    )
    ...
    def form_before_create_or_update(self, values, extra_values):
        ...
        if (
            "partner_birthdate_date" in extra_values
            and extra_values["partner_birthdate_date"] > datetime.now().date()
        ):
            raise InvalidDateException
        for key in [
            "participant_facebook_url",
            ...
            extra_values[key] = ProjectCreationWizard.sanitized_url(extra_values, key)
    ...
    def form_after_create_or_update(self, values, extra_values):
        ...
        if extra_values.get("partner_image"):
            extra_values["partner_image"] = compress_big_images(
                extra_values["partner_image"], 800, 800
            )
