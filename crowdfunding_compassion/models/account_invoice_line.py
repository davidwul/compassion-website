...
class AccountInvoiceLine(models.Model):
    ...
    def create(self, vals):
        ...
        if vals.get("campaign_id"):
            analytic = self.env["account.analytic.account"].search(
                [("campaign_id", "=", vals["campaign_id"])], limit=1
            )
            if not analytic:
                campaign = self.env["utm.campaign"].browse(vals["campaign_id"])
                analytic = analytic.create(
                    {"name": campaign.name, "campaign_id": campaign.id}
                )
    ...
    def _update_utm_data(self):
        if self.crowdfunding_participant_id:
            ...
            self.campaign_id = self.crowdfunding_participant_id.project_id.campaign_id
    ...
    def _get_ambassador_receipt_config(self):
        ...
        if crowdfunding_event:
            return self.env.ref("crowdfunding_compassion.config_donation_received")
