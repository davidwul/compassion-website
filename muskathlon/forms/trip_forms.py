    ...
    def _form_fieldsets(self):
        return [
            {"id": "tshirt", "fields": ["t_shirt_size"]},
            {
                ...
                "description": _(
                    "Please indicate a contact in case of " "emergency during the trip."
                ),
                "fields": [
                    "emergency_relation_type",
                    "emergency_name",
...
class FlightDetailsForm(models.AbstractModel):
    ...
    def _get_date(self, fname):
        if self.main_object:
            ...
            registration = self.env["event.registration"].browse(self.registration_id)
            if registration:
    ...
    def form_msg_success_created(self):
        if self.flight_type == "outbound":
            return _("Outbound flight entered. " "Please enter now the return flight.")
