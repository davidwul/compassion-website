...
class OrderMaterialForm(models.AbstractModel):
    ...
    flyer_german = fields.Selection(
        flyers_select, string="Number of flyers in german", default=0
    )
    flyer_french = fields.Selection(
        flyers_select, string="Number of flyers in french", default=0
    )
    ...
    def create_description(material, values, languages=["french", "german"]):
        ...
        for lang in languages:
            if int(values[f"{material}_{lang}"]) > 0:
                lines.append(
                    f"<li>{values[f'flyer_{lang}']} <b>{material}</b> in {lang}</li>"
                )
    ...
    def form_check_empty_value(self, fname, field, value, **req_values):
        ...
        is_valid |= (
            int(req_values["flyer_french"]) + int(req_values["flyer_german"]) <= 0
        )
        return is_valid
    ...
    def form_after_create_or_update(self, values, extra_values):
        super(OrderMaterialForm, self).form_after_create_or_update(values, extra_values)
